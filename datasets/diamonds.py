...
class Diamonds(BaseDataset):
    ...
    def _download_data(self, target_to_one_hot=True):
        ...
        if exists(self.arff_file):
            ...
            y = df[6].values.astype(float)
            x = df.drop([6], axis=1).values
            del df
