...
class Pakinsons(BaseDataset):

    def __init__(self, cache_folder:str, config:dict, pool_rng, encoded:bool,
                 data_file="pakinsons_al.pt"):
        self.raw_zip_file = os.path.join(cache_folder, "pakinsons_raw.zip")
        self.raw_unzipped_file = os.path.join(cache_folder, "pakinsons_raw/parkinsons_updrs.data")
        super().__init__(cache_folder, config, pool_rng, encoded, data_file)


    def _download_data(self, target_to_one_hot=True):
        train_url = "https://archive.ics.uci.edu/static/public/189/parkinsons+telemonitoring.zip"
        ...
        if not exists(self.raw_unzipped_file):
            ...
            z.extract('parkinsons_updrs.data', os.path.join(self.cache_folder, "pakinsons_raw"))

        if exists(self.raw_unzipped_file):
            data = pd.read_csv(self.raw_unzipped_file, header=0, index_col=None)
            data = pd.get_dummies(data, columns=["subject#"], prefix="id")
            for c in data.columns:
                if c.startswith("id"):
                    data[c] = data[c].astype(int)
            y = data["total_UPDRS"].values
            x = data.drop(["motor_UPDRS", "total_UPDRS"], axis=1).values
            del data
            ids = np.arange(len(x))
